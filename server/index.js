...
app.post('/api/trades', upload.fields([
  ...
  // Normalize date: strip time portion if ISO timestamp sent (e.g. '2026-01-03T21:00:00.000Z' → '2026-01-03')
  const tradeDate = data.date ? data.date.toString().slice(0, 10) : null;
  // Normalize risk_percent: clamp to DECIMAL(8,4) max (~9999.9999)
  const riskPercent = Math.min(parseFloat(data.risk_percent) || 0, 9999.9999);
  ...
  await pool.query(
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [
      id, USER_ID, data.account_id, data.trade_id, tradeDate, data.session, data.pair, data.direction,
      data.lot_size || 0, data.entry_price || 0, data.stop_loss || 0, data.take_profit || 0, data.exit_price || null,
      data.risk_amount || 0, riskPercent, data.account_size || 0,
...
app.put('/api/trades/:id', async (req, res) => {
  ...
  for (const f of directFields) {
    if (updates[f] !== undefined) {
      let val = updates[f];
      // Normalize date: strip time portion if ISO timestamp sent
      if (f === 'date' && val) {
        val = val.toString().slice(0, 10);
      }
      // Normalize risk_percent: clamp to DECIMAL(8,4) max
      if (f === 'risk_percent' && val !== null && val !== undefined) {
        val = Math.min(parseFloat(val) || 0, 9999.9999);
      }
      fields.push(`${f} = ?`);
      values.push(val);
    }
...
// ══════════════════════════════════════════════════════════════
app.post('/api/upload-screenshot', upload.fields([
  { name: 'before', maxCount: 1 },
  { name: 'after', maxCount: 1 },
  { name: 'file', maxCount: 1 },
]), (req, res) => {
  // Accept whichever field was sent: 'before', 'after', or legacy 'file'
  const type = req.query.type || 'file';
  const fileArr = req.files?.[type] || req.files?.file;
  const uploaded = fileArr?.[0];
  if (!uploaded) return res.status(400).json({ error: 'No file' });
  const url = `/screenshots/${req._screenshotRelPath}/${uploaded.filename}`;
  res.json({ url });
});

// ══════════════════════════════════════════════════════════════
// GLOBAL ERROR HANDLER — prevents server crash on DB errors
// ══════════════════════════════════════════════════════════════
...
// Catch unhandled promise rejections (e.g. DB connection drops)
process.on('unhandledRejection', (reason) => {
  console.error('⚠️  Unhandled rejection (server keeps running):', reason?.message || reason);
});
...
║   Network: http://192.168.0.14:${PORT}/api/health    ║
╚═══════════════════════════════════════════════════╝
  `);
});
